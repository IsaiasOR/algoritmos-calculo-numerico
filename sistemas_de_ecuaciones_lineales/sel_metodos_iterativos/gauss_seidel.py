# Método de Gauuss-Seidel para resolver sistemas de ecuaciones lineales
import numpy as np
...
def gauss_seidel(A: np.ndarray, B: np.ndarray, x0: np.ndarray, tolerancia: float, max_iteraciones: int) -> np.ndarray:
  # Método de Gauuss-Seidel para resolver Ax = B
  ...
  while errado > tolerancia and iteracion < max_iteraciones:
    for i in range(n):
      """"
      x_nuevo[:i]: Toma los coeficientes de la matriz $A$ que están a la izquierda de la diagonal principal y los multiplica por las variables x_nuevo. Estos son los valores que ya fueron actualizados en las iteraciones anteriores del mismo ciclo for.
      x[i+1:]: Toma los coeficientes a la derecha de la diagonal y los multiplica por las variables x. Estos son los valores antiguos de la iteración pasada, ya que el algoritmo aún no ha llegado a esa posición i para actualizarlos.
      """
...
# Método de Gauuss-Seidel
solucion = gauss_seidel(a_nuevo, b_nuevo, x0, tolerancia, max_iteraciones)
# Solución final
print('X: ',solucion)
print("\n" + "*"*80)